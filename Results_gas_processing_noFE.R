...
# --------
#prj<-rgcam::addMIBatchCSV(fn = "results_noFE_local.csv",
#                     proj = "results_gas_noFE.dat",
#                  clobber = FALSE)
#
#saveProject(prj,"results_gas_noFE.dat")


# --------
# Load project file
prj <- loadProject("results_gas_noFE.dat")
listScenarios(prj)
...
gdppc<-getQuery(prj,"GDP per capita MER by region") %>%
  mutate(scenario = if_else(scenario == "NewGasCalib_EUpre55CP_noFE", "CP_Default_noFE", scenario),
         scenario = if_else(scenario == "NewGasCalib_EUpre55CP_NDC_noFE", "NDC_Default_noFE", scenario),
         scenario = if_else(scenario == "NewGasCalib_EUpre55CP_NDC_noRusGas_noFE", "NDC_NoRus_noFE", scenario),
         scenario = if_else(scenario == "NewGasCalib_EUpre55CP_NDC_noRusGas_Dom_noFE", "NDC_NoRus_Dom_noFE", scenario),
         scenario = if_else(scenario == "NewGasCalib_EUpre55CP_NDC_noRusGas_Imp_noFE", "NDC_NoRus_Imp_noFE", scenario))
...
ggsave("figures/NoFE/gas_prod_ImpPipe_ImpLNG.tiff", gas.plot, "tiff", dpi = 200)
...
ggsave("figures/NoFE/gas_ImpPipe_byPipe.tiff", gas.pipelines.plot, "tiff", dpi = 200)
...
ggsave("figures/NoFE/GasPrice_pipeline.tiff", price.pipeline, "tiff", dpi = 200)
...
ggsave("figures/NoFE/GasPrice_LNG.tiff", price.lng, "tiff", dpi = 200)
...
ggsave("figures/NoFE/tfe_bySector_byReg_2030.tiff", tfe.fuel.plot, "tiff", dpi = 200)
...
ggsave("figures/NoFE/tfe_bySector_byReg.tiff", tfe.plot, "tiff", dpi = 200)
...
ggsave("figures/NoFE/co2_byReg.tiff", co2.plot, "tiff", dpi = 200)
...
ggplot(data = filter(cum_global_total_add, year <= 2050),
  ...
  ggsave(paste0("figures/NoFE/","cum_global_total_add_facet_export.pdf"),last_plot(),width=11, height=8, units="in")
...
ggplot(data = filter(cum_global_total_add, year == 2050),
  ...
  scale_fill_manual(values = c(
    ...
    "traded RUS pipeline gas" = "#D55E00"))+
  coord_flip()
  ggsave(paste0("figures/NoFE/","cum_global_total_add_facet_export_2050.pdf", sep = ""),width=6, height=3, units="in")
...
ggplot(data = filter(cum_global_total_ret, year <= 2050),
  ...
  scale_fill_manual(values = c(
    ...
    "traded PAC pipeline gas" = "#0072B2",
    "traded RUS pipeline gas" = "#D55E00"))
  ggsave(paste0("figures/NoFE/","cum_global_total_ret_facet_export.pdf", sep = ""),width=11, height=8, units="in")
...
ggplot(data = filter(cum_global_total_ret, year == 2050),
  ...
  scale_fill_manual(values = c(
    ...
    "traded RUS pipeline gas" = "#D55E00"))+
  coord_flip()
  ggsave(paste0("figures/NoFE/","cum_global_total_ret_facet_export_2050.pdf", sep = ""),width=6, height=3, units="in")
